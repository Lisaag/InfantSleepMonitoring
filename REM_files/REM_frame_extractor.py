"""
This script is used to extract frames, and crop eyes from a REM fragment.
...
def remove_folder_recursively(folder_path):
    """
    Removes all content of a given folder

    Parameters:
    folder_path: path to remove contents of
    """
    try:
...
def xyxy_to_square(x1, y1, x2, y2, size, offset = [0, 0]):
    ...
    Parameters:
    x1, y1, x2, y2: x and y bounds of the bounding box
    size: size of the output bounding box
    offset: offset from center (used for data augmentation)
    """
...
def center_pos_frames(df_bboxes,  min_bounds, max_bounds, pos_offset = 0, size_factor = 1):
    """
    extract eyes using localized position of center frame (see paper)

    Parameters:
    df_bboxes: dataframe of bbox data
    min_bounds: first frame
    max_bounds: last frame
    pos_offset: offset from center (used for data augmentation)
    size_factor: factor of bbox size, to increase or decrease cut out size (used for data augmentation)
    """
...
def interpolate_pos_frames(df_bboxes,  min_bounds, max_bounds, pos_offset = 0, size_factor = 1):
    """
    extract eyes using localized position of first and last frame, and interpolating for in between frames (see paper)
...
def every_pos_frames(df_bboxes,  min_bounds, max_bounds, pos_offset = 0, size_factor = 1):
    """
    extract eyes using all localized positions(see paper)
...
def save_frame_stack(frame, vid, current_frame, bbox, dir):
    ...
    Parameters:
    frame: reference to frame
    vid: reference to video
    current_frame: index of current frame that is processed
    bbox: bbox x, y min max pos info
    dir: save dir path
    """
    x1, y1, x2, y2 = bbox
...
def extract_frames(video_dir:str, file_name:str, csv_dir:str, patient_id:str, REMclass:str, cropped_dir:str, suffix:str="", temp_aug_offset=[3, 3], pos_aug_offset=0, size_factor=1):
    """
    extract eyes from video fragments. Save as stack of frames

    Parameters:
    video_dir: dir of video fragment
    file_name: file name of video fragment
    csv_dir: path of csv, containing information about fragment
    patient_id: id of patient in video
    REMclass: class of video fragment, either O, OR, C, or CR
    cropped_dir: dir where cropped frames are saved to
    suffix: suffix used to distinguish augmented data
    temp_aug_offset: temporal augmentation offset, crops video a few frames sooner or later than original
    pos_aug_offset: position offset, used for position augmentation
    size_factor: cropping size factor, used for size data augmentation
    """
...
def detect_vid():
    """
    extract stack of frames from fragments. stack of frames is then used to train REM model. stack of frames is of class O, OR, C, or CR
    """
