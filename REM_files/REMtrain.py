...
def create_model(lr = 0.0001, dropout=0.3, l2=0.1, input_shape=(1, 6, 64, 64), seed = 0):
    model = models.Sequential([
        ...
        layers.Conv3D(16, kernel_size=(3, 3, 3), activation='relu', padding='valid'),
        layers.Dropout(dropout, seed=seed),
        layers.MaxPooling3D(pool_size=(2, 2, 2)),
       
        layers.Conv3D(32, kernel_size=(3, 3, 3), activation='relu', padding='same', strides = (1, 2, 2)),
        layers.Conv3D(32, kernel_size=(3, 3, 3), activation='relu', padding='same', strides = (1, 2, 2)),
        layers.Dropout(dropout, seed=seed),
        layers.MaxPooling3D(pool_size=(2, 2, 2)),

        layers.Flatten(),
        layers.Dense(32, activation='relu', kernel_regularizer=regularizers.L2(l2), kernel_initializer=tf.keras.initializers.HeNormal(seed=seed)),
