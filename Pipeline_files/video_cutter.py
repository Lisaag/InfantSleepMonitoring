...
def get_boxes(df_bboxes, fragment_idx):
    ...
    for i in range(curr_starting_frame, curr_starting_frame + settings.fragment_length):
        ...
        confs:dict = ast.literal_eval(row["confs"])
        classes:dict = ast.literal_eval(row["classes"])
        boxes:dict = ast.literal_eval(row["boxes"])
...
df_bboxes = pd.read_csv(os.path.join(settings.eye_loc_path, settings.cur_vid))
